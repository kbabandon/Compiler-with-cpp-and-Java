...
//    布尔因子
    ...
    //    处理布尔表达式
    public static void bool_expression() {//先或后与
        space++;
        output("开始布尔表达式");
        bool_item();  //处理布尔项
        while (true) {
...
//                开始判断是否为表达式 算术|关系|布尔|赋值
    ...
    //表达式语句
    public static void expression() {//表达式 调用了赋值表达式
...
//            可能是函数声明也可能是变量声明，需根据标识符后面是否是括号来进一步确认
            token = getNextToken();//这里可以判断是否是标识符
            token = getNextToken();
//            有括号肯定是函数声明，
            if (token == Compile.dictionary.get("(")) {
                index -= 2;
